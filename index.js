...
client.on('guildMemberUpdate', (member, guild) => {
  if (guild.members.filter(mem => mem.user.username.startsWith("!")).then(setNickname(`${guild.member.username.substr(2, 100)}`, `Attempting to hoist on the user list.`)))
});
